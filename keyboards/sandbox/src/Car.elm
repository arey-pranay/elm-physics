            ...
            { model
                ...
                , world =
                    ...
                    model.world
                        ...
                        |> World.update (applySteer model.steering baseFrame)
...
initialWorld =
    World.empty
        ...
        |> World.add floor
        |> addBall (Point3d.meters 0 0 0)


addBall : Point3d Meters WorldCoordinates -> World Data -> World Data
addBall offset world =
    world
...
applySpeed speed baseFrame body =
    if speed /= 0 && (Body.data body).name == "wheel3"  then
        let
            forward =
...
applySteer : Float -> Frame3d Meters WorldCoordinates { defines : BodyCoordinates } -> Body Data -> Body Data
applySteer steer baseFrame body =
    if steer /= 0 && (Body.data body).name == "wheel3"  then
        let
            left =
                Frame3d.xDirection baseFrame
        ...
        in
        body
            |> Body.applyForce (Force.newtons (-steer * 100)) left pointOnTheWheel
            |> Body.applyForce (Force.newtons (-steer * 100)) (Direction3d.reverse left) pointUnderTheWheel
...
base =
    let
        bottom =
            Block3d.centeredOn
                Frame3d.atOrigin
                ( Length.meters 0, Length.meters 0, Length.meters 0 )
