module Projects.Table exposing (main)
...
-- Id can be Mouse or Floor or Table
type Id
    = Mouse
    | Floor
    | Table

--We're creating A type called "Model", it has world, width, height, maybeRaycastResult
type alias Model =
    { world : World Id
...
--Msg is generally what we get as an update, it can be AnimationFrame, or resize, mousedown, mouseup or mousemove
type Msg
    = AnimationFrame
    | Resize Int Int
...
--this is where the program starts, it has Model Msg
...
--main has initialization, update, subsc, and view.. Understandable, have a nice day

init : () -> ( Model, Cmd Msg ) --defining the initial Model and Msg
init _ =
    ( { world = initialWorld
      , width = pixels 0
      , height = pixels 0
...
--subscribing to the model to get continuous responses
...
--defining tableBlocks, it is a list of 3 things. a ":" b means a has a type of b
