...
import Length exposing (Meters, millimeters, inMeters)
...
slope =
    ...
    in
        ...
        Body.block slopeBlock (Obstacle slopeBlock)
            |> Body.rotateAround Axis3d.y (Angle.degrees 30)
            |> Body.moveTo (Point3d.millimeters 100 100 100)



        -- Body.block slopeBlock (Obstacle slopeBlock)
        --     |> Body.rotateAround Axis3d.x (Angle.degrees 120)
        --     |> Body.moveTo (Point3d.millimeters 0 0 0)
    

circleList : List (Sphere3d.Sphere3d Meters BodyCoordinates) --i.e., a Sphere3d object can handle meters and bodycoordinates
circleList =
    [ Sphere3d.atPoint (Point3d.millimeters 200 200 0)
        (Length.millimeters 100) --try adding more elements to this list by looking at the table
    ]
